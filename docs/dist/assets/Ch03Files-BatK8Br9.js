import{_ as n,c as a,a as c,o}from"./index-B3LiEHuY.js";const e={};function p(m,s){return o(),a("section",null,[...s[0]||(s[0]=[c(`<h2><span class="no">03</span>目录结构与文件作用</h2><pre><code>myblog/
...
└── pic/              <span class="comment"># 原始头像素材</span></code></pre><h3>后端文件调用关系（按启动顺序）</h3><p><code>main.go</code> → 调 <code>db.go</code> 的 <code>initDB</code>（连库建表）→ 调 <code>auth.go</code> 的 <code>ensureAdmin</code>（建管理员）→ 注册路由（处理器函数都在 <code>auth.go</code> / <code>posts.go</code> / <code>upload.go</code>）→ <code>static.go</code> 托管前端页面 → 监听端口。</p>`,4)])])}const l=n(e,[["render",p]]);export{l as default};
